...
class Preprocess:
    def __init__(self, target,  dsd_path, data_path, n_fft = 1024, hop_length = 256, segment_length = 0.5, sr = 22050):
        '''
    ...
    def spec(self, sliced_array, spec_type):
      '''
      ...
      - spec_type: (str)
      Type of spectrogram to be extracted. Currently only allowing 'mel_spec' and
      'spec'.
    ...
    def preprocess(self, dev_test, spec_type):
        '''
        ...
        for items in sorted(os.walk(mix_path)):
            ...
            if items[0] is not mix_path:
                ...
                for s in range(num_segments):
                    ...
                    self.data['Mixture'].append(self.spec(fileM[start:end], spec_type))
                    self.data['Target'].append(self.spec(fileT[start:end], spec_type))
